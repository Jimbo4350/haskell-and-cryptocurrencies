...
sumMany' = do
    putStrLn "How many numbers do you want to add?"
    n  <- readLn :: IO Int
    xs <- mapM (\i -> putStrLn ("Enter number " ++ show i ++ " of " ++ show n ++ ":") >> readLn) [1 .. n]
    putStrLn ("The sum of all numbers is " ++ show (sum xs :: Int) ++ ".")
...
wc file = do
   text <- readFile file
   let ws = length (words text)
       ls = length (lines text)
       cs = length text
   return (ws, ls, cs)
...
dice :: Dice -> IO Int
dice (D q d)    = liftM sum (replicateM q (randomRIO (1, d)))
dice (Const n)  = return n
dice (Plus d e) = liftM2 (+) (dice d) (dice e)
...
diceRange (D q d)    = (q, q * d)
diceRange (Const n)  = (n, n)
diceRange (Plus d e) =
    let (minD, maxD) = diceRange d
        (minE, maxE) = diceRange e
    in  (minD + minE, maxD + maxE)
...
extract i (x : xs) =
    ...
    in  (y, x : ys)
extract _ []       = error "invalid index or empty list"
...
httpTest :: IO [String]
httpTest = do
    h <- connectTo "www.haskell.org" (PortNumber 80)
    hPutStrLn h "GET /index.html HTTP/1.1"
    hPutStrLn h "Host: www.haskell.org"
    hPutStrLn h ""
    xs <- hGetLines h
    hClose h
    return xs
...
withConnection :: HostName -> PortID -> (Handle -> IO r) -> IO r
withConnection host port = bracket (connectTo host port) hClose

httpTest' :: IO [String]
httpTest' = withConnection
    "www.haskell.org"
    (PortNumber 80)
    (\h -> do
        hPutStrLn h "GET /index.html HTTP/1.1"
...
dnsTest = do
    ...
    withResolver rs (\r -> lookupA r (pack "seed.bitcoin.sipa.be"))
